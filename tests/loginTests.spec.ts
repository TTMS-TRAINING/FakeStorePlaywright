...
test.describe('Account Page Tests', () => {
  ...
  test('wrong email login', async ({ page }) => {
    ...
    await expect(
      page.getByText(
        'Błąd: dla adresu e-mail ' + AccountTestData.WrongEmailLogin.username + ' podano nieprawidłowe hasło. Nie pamiętasz hasła?'
      )
    ).toBeVisible();
  });
  ...
  test('wrong login', async ({ page }) => {
    ...
    await expect(
      page.getByText('Błąd: wpisano niepoprawne hasło dla użytkownika ' + AccountTestData.WrongLogin.username + '. Nie pamiętasz hasła?')
    ).toBeVisible();
  });
  test('not registered user', async ({ page }) => {
    ...
    await expect(
      page.getByText(
        'Błąd: brak ' +
          AccountTestData.NotRegistered.username +
